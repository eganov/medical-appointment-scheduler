...
import requests
from personnel_appointments.test.resource import MOCK_DEPARTMENTS
from personnel_appointments.test.testing_utils import create_missing_departments, next_mock_employee, next_mock_user, \
...
# python manage.py test personnel_appointments


class EmployeeApiTest(TestCase):
    """
    API test case for the `personnel_appointments.views.EmployeeViewSet` endpoints.
    """
...
def create_employee_request(employee: Dict[str, str]) -> Response:
    print(
        f'Create new employee "{employee["first_name"]} {employee["last_name"]}" with employee ID '
...
class AccountApiTest(TestCase):
    """
    API test case for the `personnel_appointments.views.AccountViewSet` endpoints.
    """
